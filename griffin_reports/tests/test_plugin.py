...
# Griffin-IDE and Local imports
from griffin.utils.programs import TEMPDIR
from griffin_reports.reportsplugin import ReportsPlugin
from griffin_reports.utils import WELCOME_PATH
...
def setup_reports(qtbot, monkeypatch):
    ...
    monkeypatch.setattr(
            'griffin_reports.reportsplugin.ReportsPlugin.initialize_plugin',
            lambda self: None)
...
def test_check_compability(qtbot, setup_reports, monkeypatch):
    ...
    monkeypatch.setattr('griffin_reports.reportsplugin.PYQT4', True)
...
def test_render_tmp_dir(qtbot, setup_reports, report_file):
    """Test that rendered files are created in griffin's tempdir."""
    reports = setup_reports
    output_file = reports._render_report(report_file)

    # Test that outfile is in griffin tmp dir
...
def test_save_report(qtbot, tmpdir_factory, setup_reports, report_file,
    ...
    monkeypatch.setattr('griffin_reports.reportsplugin.getexistingdirectory',
    ...
    monkeypatch.setattr('griffin_reports.reportsplugin.copy_tree',
                        raise_exception)
